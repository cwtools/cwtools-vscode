...
let graphNodeToJson (node: GraphNode) =
    ...
    let referenceToJson (ref: GraphReference) =
        ...
        |] |> Array.choose id |> FSharp.Data.JsonValue.Record
    ...
    let detailsToJson (details: Map<string, string list>) =
        ...
        |> Array.map (fun (k, vs) -> 
            FSharp.Data.JsonValue.Record [| 
                ...
                "values", (vs |> Array.ofList |> Array.map FSharp.Data.JsonValue.String |> FSharp.Data.JsonValue.Array)
            |])
        |> FSharp.Data.JsonValue.Array
    
    [|
        Some ("id", FSharp.Data.JsonValue.String node.id)
        node.displayName |> Option.map (fun s -> ("name", FSharp.Data.JsonValue.String s))
        ...
        node.documentation |> Option.map (fun s -> "documentation", FSharp.Data.JsonValue.String s)
        ...
        node.entityTypeDisplayName |> Option.map (fun s -> ("entityTypeDisplayName", FSharp.Data.JsonValue.String s))
        node.abbreviation |> Option.map (fun s -> ("abbreviation", FSharp.Data.JsonValue.String s))
    |] |> Array.choose id |> FSharp.Data.JsonValue.Record
