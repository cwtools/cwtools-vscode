...
module LanguageServerFeatures =
    ...
    let hoverDocument (eu4GameObj) (hoi4GameObj) (stlGameObj) (ck2GameObj) (irGameObj) (vic2GameObj) (client : ILanguageClient) (docs : DocumentStore) (doc : Uri)  (pos: LSP.Types.Position) =
        async {
            ...
            let hoverFunction (game : IGame<_, _>) =
            ...
            return
                match stlGameObj, hoi4GameObj, eu4GameObj, ck2GameObj, irGameObj, vic2GameObj with
                |Some game, _, _, _, _, _ -> hoverFunction game
                |_, Some game, _, _, _, _ -> hoverFunction game
                // |_, Some game, _ ->
                ...
                //     |None ->
                //         { contents = MarkupContent ("markdown", ""); range = None }
                |_, _, Some game, _, _, _ -> hoverFunction game
                |_, _, _, Some game, _, _ -> hoverFunction game
                |_, _, _, _, Some game, _ -> hoverFunction game
                |_, _, _, _, _, Some game -> hoverFunction game
